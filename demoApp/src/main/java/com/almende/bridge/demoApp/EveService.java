...
public class EveService extends Service implements GooglePlayServicesClient.ConnectionCallbacks,
        GooglePlayServicesClient.OnConnectionFailedListener {
    public static final HandlerThread myThread = new HandlerThread(
            EveService.class.getCanonicalName());
        ...
        myHandler.post(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    System.err.println("Couldn't start AndroidIntentStateFactory!");
                    e.printStackTrace();
                }
                ...
                XmppService xmppService = new XmppService(host, hostUrl, port, serviceName);
                ...
                try {
                    if (host.hasAgent(DEMO_AGENT) && host.getAgent(DEMO_AGENT) != null) {
                        ...
                        if (!"BridgeDemoAgent".equals(test.getType())
                                || !test.getVersion().equals(BridgeDemoAgent.getBaseVersion())) {
                            System.err.println("Warning: replacing agent " + DEMO_AGENT
                                    + " with new code version " + BridgeDemoAgent.getBaseVersion());
                        ...
                        agent = host.createAgent(BridgeDemoAgent.class, DEMO_AGENT);
                    }
                } catch (Exception e) {
                ...
                if (agent != null) {
                    ...
                    System.err.println("Agent is still null, not reconnecting.");
                }
                ...
                if (agent != null) {
                    try {
                        ...
                        EventBus.getDefault().post(new StateEvent(null, "agentsUp"));
                    } catch (Exception e) {
                        System.err.println("Failed to init agent.");
                        e.printStackTrace();
                    }
                } else {
                    System.err.println("Agent is still null, not setting up task");
                }
            }
        });
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (servicesConnected()) {
            EveService.mLocationClient = new LocationClient(this.getApplicationContext(), this,
                    this);
    ...
    public void onDestroy() {
        ...
        if (EveService.mLocationClient != null)
            EveService.mLocationClient.disconnect();
    }

    public void onEventAsync(StateEvent event) {
        System.err.println("Service received StateEvent:" + event.getValue() + " threadId:"
                + Thread.currentThread().getId());

        if (event.getValue().equals("newTask") && event.getAgentId().equals(EveService.DEMO_AGENT)) {
            try {
    ...
    public void onConnectionFailed(ConnectionResult connectionResult) {
        Log.d("Google Play Services", "Google Play Services can't be connected?");
    }
